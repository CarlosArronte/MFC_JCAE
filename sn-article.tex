...
\abstract{This paper investigates a model-free unified chassis controller (UCC) for the simultaneous regulation of vehicle side-slip angle, yaw rate, and roll angle during abrupt maneuvers. The proposed strategy combines iPD-based model-free controllers with a control-allocation architecture and is evaluated in a MATLAB/Simulink\textsuperscript{\textregistered}-CarSim\textsuperscript{\textregistered} co-simulation environment under a sine-with-dwell maneuver for three road-friction conditions. The comparison includes the open-loop vehicle, the built-in CarSim\textsuperscript{\textregistered} ESC baseline, a fixed-parameter 2PID+fuzzy controller, and a gain-scheduled LQR benchmark. The results show that no benchmark minimizes every nominal metric at all friction levels. However, the proposed controller provides the most consistent robustness across the full range of conditions. In particular, it is the only strategy that completes the low-friction case while preserving bounded and meaningful lateral and roll responses, whereas the other baselines either lose practical stability before the end of the maneuver or exhibit unacceptable state excursions. The control-effort analysis further shows that this robustness is achieved through more active brake modulation while keeping the anti-roll torque within comparatively moderate bounds. These findings indicate that the proposed MFC-based UCC offers a favorable compromise between regulation quality, practical closed-loop stability, and actuator demand under friction variations.}
...
signals in active safety systems \citep{Rajamani2012}.
...
respectively. In the present co-simulation setup, both $V_x$ and $V_y$ are directly obtained from CarSim\textsuperscript{\textregistered}. Accordingly, $\beta$ is computed from ideal state information available in simulation, rather than being reconstructed from measured signals through an estimation algorithm. This choice is suitable for comparative controller assessment, although it abstracts from the state-estimation problem that would arise in a real-vehicle implementation.
...
In this study, the road friction coefficient ($\mu$) is imposed as a simulation parameter and kept constant within each test scenario. Therefore, $\mu$ is not estimated online, but prescribed a priori in CarSim\textsuperscript{\textregistered} to define the operating condition under which the controllers are evaluated. This choice is consistent with the main objective of the experiments, namely, to assess controller performance and robustness under different known friction levels.
...
The performance of the UCC is evaluated under a SWD
...
The public CarSim\textsuperscript{\textregistered} description documents the test procedure and the example 
...
\citep{carsim_fmvss126}. For that reason, the CarSim\textsuperscript{\textregistered} baseline should be 
...
in \citep{arronte2024controle}. Public CarSim\textsuperscript{\textregistered} documentation states that the 
...
internally by CarSim\textsuperscript{\textregistered}.
...
Figure~\ref{fig:mu_03_zoom} provides a closer view of the most critical interval of the icy-road maneuver and helps explain why the proposed controller is the only one that preserves meaningful regulation until the end of the test. In the lateral variables, the zoomed panels show that the MFC keeps the yaw-rate trajectory better centered around the reference while maintaining a compact side-slip response during the transient in which the benchmark controllers begin to separate. The 2PID+fuzzy controller remains competitive in some local amplitudes, but its response is less robust over time, whereas the GS-LQR already exhibits the onset of the oscillatory growth that later becomes dominant in the full-scale plots. The roll-angle and roll-rate zooms reinforce the same interpretation: the proposed MFC keeps the vertical response smoother and more symmetric around the equilibrium, while the CarSim\textsuperscript{\textregistered} ESC and the GS-LQR reveal an earlier loss of damping. Therefore, the zoomed figure complements Figure~\ref{fig:mu_03} by showing that the advantage of the MFC at $\mu = 0.3$ is not only a matter of surviving longer, but also of preserving better local transient regulation in the interval where the maneuver becomes critical.


Figure~\ref{fig:Control_mu_03} shows the brake-pressure distribution at each wheel for the most demanding case, namely $\mu = 0.3$. Beyond the scalar metrics reported later in Tables~\ref{tab:control_effort_pbs} and \ref{tab:control_effort_mx}, these plots reveal clear differences in the temporal structure of the control action. In Figure~\ref{fig:Carsim_03}, the CarSim\textsuperscript{\textregistered} ESC concentrates most of the intervention on the right-rear wheel, with a long ramp followed by late pulses, while the remaining wheels are only marginally used. Figure~\ref{fig:LQR_03} shows that the GS-LQR relies on sparse and narrow pulses, first concentrated on the left-rear wheel and later shifted to the left-front wheel, which is indicative of a highly impulsive and weakly distributed braking strategy. Figure~\ref{fig:PID_03} exhibits a more regular sequence of moderate-amplitude pulses, mainly on the left side, but this action stops before the end of the test, consistently with the fact that the controller does not complete the maneuver. By contrast, Figure~\ref{fig:MFC_03} shows that the proposed iPD-based MFC maintains a sustained and coordinated multiwheel modulation over the whole maneuver, with repeated bounded pulses distributed among the four brakes.

This figure is consistent with, but not redundant with, the discussion of Tables~\ref{tab:control_effort_pbs} and \ref{tab:control_effort_mx}. The brake-pressure table quantifies that, for $\mu = 0.3$, the proposed controller does not minimize RMS, energy, or saturation percentage; the figure explains why, since the MFC deliberately applies a persistent brake modulation in order to preserve bounded vehicle states until the end of the maneuver. Conversely, the more intermittent patterns of the CarSim\textsuperscript{\textregistered} ESC, GS-LQR, and 2PID+fuzzy controllers help explain why they either lose practical stability early or fail to maintain acceptable state regulation. At the same time, the anti-roll torque table shows that this more active brake intervention is not accompanied by the largest anti-roll demand. Therefore, the combined reading of Figure~\ref{fig:Control_mu_03} and Tables~\ref{tab:control_effort_pbs}--\ref{tab:control_effort_mx} supports the same conclusion from two complementary viewpoints: in the icy-road case, the MFC attains the best robustness by replacing sporadic high-impact interventions with a more sustained and coordinated use of the brake channel, while keeping the anti-roll action within comparatively moderate levels.
